...
def dict_apply(
    ...
    for key, value in x.items():
        ...
        # only apply to torch tensors
        elif torch.is_tensor(value):
            result[key] = func(value)

        # (optional) convert numpy arrays to torch first, then apply
        elif isinstance(value, np.ndarray):
            result[key] = func(torch.from_numpy(value))

        # skip lists / strings / None / numbers
        else:
            result[key] = value
    return result
