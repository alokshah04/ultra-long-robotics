...
def get_language_goal(self, env):
    ...
    elif env == "MemoryChain":
        target_color = getattr(self.env.unwrapped, "target_cube_color", "unknown")
        num_repeats = getattr(self.env.unwrapped, "num_distractor_repeats", 1)
        num_buttons = len(getattr(self.env.unwrapped, "procedure_order", []))
        word = num2words.get(num_repeats, str(num_repeats))
        button_word = num2words.get(num_buttons, str(num_buttons))
        language_goals.append(
            f"watch which cube is highlighted and remember it, watch it get hidden under a container, "
            f"then pick up and place the distractor cube {word} times, press the {button_word} procedure buttons "
            f"in the shown order, press the recall button, retrieve the container hiding the {target_color} cube, "
            f"and finally place the {target_color} cube onto the final target"
        )
        language_goals.append(
            f"remember the highlighted cube and the container it is hidden under, complete the unrelated "
            f"pick-and-place and button-sequence tasks, then press the recall button and retrieve the "
            f"{target_color} cube to place it on the final target"
        )
